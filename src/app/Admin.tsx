...
import {
  ...
  ChevronUp, Check, X, Eye, Plus, Trash2
} from "lucide-react";
import { api } from "./api";
      ...
      <div className="flex items-start justify-between p-5 gap-4">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            <span className="font-bold text-gray-800 truncate">{record.dadosPessoais?.nome || record.tutor?.nome ||record.padrinho?.nome ||record.nome ||record.animalNome ||record.endereco ||`#${record.id?.slice(0, 8)}`}</span>
...
export default function Admin({ showToast }: Props) {
  ...
  const loadTab = async (t: AdminTab) => {
    ...
    setData([]);
    try {
      const loaders: Record<AdminTab, (() => Promise<any>) | null> = {
        dashboard: null,
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                {data.length === 0 && !loading && (
                  <div className="col-span-3 text-center py-16 text-muted-foreground">
                    ...
                    <button onClick={() => setShowNewAnimal(true)} className="mt-3 text-green-600 font-semibold hover:underline">Cadastrar o primeiro animal</button>
                  </div>
                )}
              </div>
            )}
          ...
          <div>
            ...
            {loading ? <div className="flex justify-center py-16"><Loader2 size={32} className="animate-spin text-amber-500" /></div> : (
              <div className="space-y-3">
                ...
                {data.length === 0 && <div className="text-center py-16 text-muted-foreground"><AlertTriangle size={40} className="mx-auto mb-3 opacity-30" /><p>Nenhum resgate registrado.</p></div>}
              </div>
            )}
          </div>
        )}
  ...
  <div>
    <div className="flex items-center justify-between mb-5">
      <h2 className="font-bold text-xl text-gray-800">
        Denúncias ({data.length})
      </h2>

      <button
        onClick={refresh}
        className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-gray-700 font-semibold px-3 py-2 rounded-xl hover:bg-white border border-border"
      >
        <RefreshCw size={14} />
      </button>
    </div>
        ...
        {data.map((r) => (
          <RecordCard
            ...
            onStatusChange={async (id, status) => {
              try {
                await api.denuncias.setStatus(id, status);
